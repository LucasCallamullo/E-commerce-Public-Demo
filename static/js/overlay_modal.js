...
class OverlayManager {
    ...
    // agregado prueba para ver si esto si soluciona lo del popstate paralelo con pagination
    static isClosingByCode = false;
    ...
    close() {
        ...
        if (OverlayManager.overlayStack.length === 0) {
            // Marcamos que nosotros estamos provocando el popstate
            OverlayManager.isClosingByCode = true; 
            history.back();
            // history.go(-1);
            
            // Limpiamos la bandera después de un micro-delay para que 
            // los eventos de este ciclo terminen de procesarse
            setTimeout(() => {
                OverlayManager.isClosingByCode = false;
            }, 200);
        }
    }
}
